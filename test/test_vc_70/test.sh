...
if [ $main_py_status -ne 0 ]; then
    echo "⚠️ Test FAILURE [vCenter Server 7.0]: main.py exited with code $main_py_status"
    exit 1
fi
...
if [ "$checksum_file" = "$checksum_answer" ]; then
    ...
    echo "⚠️ Test FAILURE [vCenter Server 7.0]: Checksums do not match."
